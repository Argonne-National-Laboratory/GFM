...
!e and e refer to east and west points respectively
! Let:
!      h_delta = h - hw
!		 T_delta = T - Tw
! Know that:
...
c_over_T = (udf_cl(idx_east)%f-udf_cl(idx_west)%f) / (udf_cl(idx_east)%t-udf_cl(idx_west)%t)

T_delta = (-udf_cl(idx_west)%f + &
           sqrt((udf_cl(idx_west)%f*udf_cl(idx_west)%f) + (2.0d0*c_over_T*h_delta))) / c_over_T
