...
!----------------------------------------------------------------------
      ...
      DO 120 K=2,LP,2
         ...
         IF (M_S.EQ.1) THEN
            ...
            !Lg(i,j,k)%T=Lg(i,j,k)%h/Lg(i,j,k)%c !call to get T ******************
            call h_to_Tlg(Lg(i,j,k)%h,Lg(i,j,k)%T) !call to get T ******************
            ...
            if (Tmax_c > udf_cl(udf_cln)%T .and. Lg(i,j,k)%T > Tmax_c) then !maximum temperature in combustion space
               Lg(i,j,k)%T = Tmax_c
               Lg(i,j,k)%h = h_pts(udf_cln) + udf_cl(udf_cln)%f * (Tmax_c-udf_cl(udf_cln)%T) !****** 
            endif
...
!----------------------------------------------------------------------
      ...
      DO 160 K=2,LP,2
         ...
         IF (M_S.EQ.1) THEN
            LG(I,J,K)%P=LG(I1,J1,K1)%P*F1+LG(I2,J2,K2)%P*F2
            LG(I,J,K)%T=max(LG(I1,J1,K1)%T*F1+LG(I2,J2,K2)%T*F2,Tmltr)
