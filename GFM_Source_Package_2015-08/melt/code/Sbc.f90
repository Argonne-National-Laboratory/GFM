...
!------------------------
! Set up the enthalpy points array

udf=>udf_cl !udf points to the liquid glass specific heat array

allocate (h_pts(udf_cln))
h_pts = zero
h_total = udf_cl(1)%f*udf_cl(1)%t
h_pts(1) = h_total

if (udf_cln>1) then
	do n=2,udf_cln
		h_total = h_total + ((udf_cl(n-1)%f+udf_cl(n)%f)/2)*(udf_cl(n)%t-udf_cl(n-1)%t)	
		h_pts(n) = h_total
	enddo
endif
!also set the maximum enthalpy
h_Tmax =  h_total + (udf_cl(udf_cln)%f * (T_mx-udf_cl(udf_cln)%t))
         ...
         DO L=1,NPS_C
            ...
            if (iu == 1) then !charger is x-direction
               ...
               elseif (u1_pc(L,1) < zero) then         !charger is negative x-direction 
				...
				! This code handles opposed chargers by assuming that solids will melt before they 
				! reach the centerline. The simple batch model cannot handle colliding batch flows
            if (iu == 2) then !charger is y-direction
               if (u1_pc(L,2) > zero) then !charger is positive y-direction
                  do jc=jd2+1,nz
                     if (ibcell(i,jc*2,k).ne.0) exit
							if (pc(id2,jc,kd2,L)%u(2) == zero .or. y(jc*2) < y_cent_m) then 
                        pc(id2,jc,kd2,L)%u(1:3)=u1_pc(L,1:3)
                     endif
                  enddo
                  if (ibcell(i,jc*2,k) .ne. 2) then !not an inlet
                     pc(id2,jc,kd2,L)%u(1:3)=u1_pc(L,1:3) !any particles at the far wall hit it
						endif
               elseif (u1_pc(L,2) < zero) then         !charger is negative y-direction 
                  do jc=jd2-1,2,-1
                     if (ibcell(i,jc*2,k).ne.0) exit
							if (pc(id2,jc,kd2,L)%u(2) == zero .or. y(jc*2) > y_cent_m) then 
                        pc(id2,jc,kd2,L)%u(1:3)=u1_pc(L,1:3)
...
320      IF (NPS_S.LE.0) GOTO 340
         ...
         DO L=1,NPS_S
            ...
            if (iu == 2) then !charger is y-direction
               if (u1_ps(L,2) > zero) then !charger is positive y-direction
                  do jc=jd2+1,nz
                     if (ibcell(i,jc*2,k).ne.0) exit
							if (ps(id2,jc,kd2,L)%u(2) == zero .or. y(jc*2) < y_cent_m) then 
               ...
               else                        !charger is negative y-direction 
                  do jc=jd2-1,2,-1
                     if (ibcell(i,jc*2,k).ne.0) exit
							if (ps(id2,jc,kd2,L)%u(2) == zero .or. y(jc*2) > y_cent_m) then 
                        ps(id2,jc,kd2,L)%u(1:3)=u1_ps(L,1:3)
