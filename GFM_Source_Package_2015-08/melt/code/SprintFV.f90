...
!======================================================================
! SPRINTFV.F90
!     to print computed results of the system state for the melt
...
778   format(1x,e14.6)
      ...
      filename=casedir//'\FV_nodes'//runum//'m.dat'
      ...
      filename=casedir//'\FV_values'//runum//'m.dat'
      numb=5
      ...
      write(nu_prtfv2,777)(((lg(i,j,k)%t,i=3,mp-1,2),j=3,np-1,2),k=3,lp-1,2)
      write(nu_prtfv2,778)(((lg(i,j,k)%p,i=3,mp-1,2),j=3,np-1,2),k=3,lp-1,2)
      write(nu_prtfv2,778)(((lg(i,j,k)%u(1),i=3,mp-1,2),j=3,np-1,2),k=3,lp-1,2)
      write(nu_prtfv2,778)(((lg(i,j,k)%u(2),i=3,mp-1,2),j=3,np-1,2),k=3,lp-1,2) 
      write(nu_prtfv2,778)(((lg(i,j,k)%u(3),i=3,mp-1,2),j=3,np-1,2),k=3,lp-1,2)
      close(nu_prtfv2)

      ! Write out Fieldview variable names file
      filename=casedir//'\FV_names'//runum//'m.nam'
